...
**Known-failing on the OpenAI pin** — see [`FAILURES.md`](FAILURES.md) for the full account. Journey modules must be judged **one at a time** (`jido_memory_journey` 2/3, `generalise_journey` 1/2): a whole-suite `mix test.journey` reports 3/7 because each module's `setup_all` starts its own `Gralkor.Python`, whose orphan reaper SIGKILLs the previous module's live `redislite/bin/redis-server` and leaves later queries with a refused socket. The genuine failures:

- `jido_memory_journey` — recall surfaces the wrong fact, returning the vacuum-job fact that belongs to another test in the module. **Which** test fails varies run to run (round-trip and flush one run, ERL round-trip the next), so treat the names as variance rather than a fixed list; isolated runs show 2/3. Not stale storage: `setup_all` builds a fresh `System.tmp_dir!()` path per module and removes it on exit. It is semantic search quality under `text-embedding-3-small`, whose 1024 dims graphiti slices silently, where the suite was written against `gemini-embedding-2-preview`.
