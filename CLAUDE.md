...
- **Lenses** — the application registry at `config :jido_gralkor, :lenses` defines named, scoped memory channels. Appending definitions own an ontology and an ingestion module implementing `Gralkor.Lens.Ingestion.ingest/2`; replaceable definitions own a complete graph format. Every Lens resolves to the same operator-local or shared-global destination rules. `Gralkor.Client.ingest/1`, `replace/1`, and `search/1` are the direct consumer boundary. The implicit `"operator"` Lens preserves the older deployment-wide `:ontology` and operator group; concurrent search returns its results before additional-Lens results.
- **Reflections** — `config :jido_gralkor, :reflections` declares named asynchronous post-ingestion processes. Each definition points to a repository YAML multi-turn Chain of Thought and owns an `:operator` or `:global` destination named after the Reflection. A completed multi-Lens ingestion supplies all successful lensed representations; the runner interpolates each step's exact structured output into later prompts, exposes the host tools and complete tool context, and stores exactly one artefact. `generalisations` (global) and `erl` (operator-local) are the packaged defaults. Reflection search uses `Gralkor.Search.reflections` and optional `artefact_id`, independently of Lens search.
- **Lens-aware plugin flow** — mount with `:ingestion_lens` and optional `:search_lenses`. A query may override the selected ingestion Lens through `tool_context[:lens]`; the plugin retains that selection on the request-correlated Jido thread entry so terminal capture remains bound after ReAct releases its transient tool context. `memory_add` and automatic capture use that selected Lens, while `memory_search` concurrently searches distinct resolved destinations and returns the reserved `"operator"` Lens results before configured additional-Lens results. Successful flushes schedule configured Reflections only after all intended Lens writes finish.
...
- **`JidoGralkor.Actions.MemorySearch`** — the ReAct search tool. It short-circuits blank queries and missing sessions; Lens-aware mounts call `Gralkor.Client.search/1` even when their additional Lens list is empty, while implicit-operator mounts use legacy `Gralkor.Client.impl().recall/4`. Errors propagate and successful Lens results are returned as JSON identifying the searched Lens for every fact.
...
**Embedded Gralkor adapter.** `Gralkor.*` contains the compatibility adapter, independent Lens boundary, and Reflection subsystem. Lens capture uses `capture/6`, `/7`, or the internal `/8` context-carrying form, batches by Lens, and flushes through `Client.ingest_with_representation/1`. Completed lensed representations preserve a shared evidence id plus their Lens identity. The CaptureBuffer schedules Reflections asynchronously only after every intended Lens has succeeded. The implicit-operator compatibility path remains `capture/5`, `memory_add/3`, `recall/4`, and deployment-wide `:ontology`; it does not produce Reflection input.
...
**Lenses.** `:lenses` is a list of application-owned keyword definitions. Appending Lenses use `:name`, `:scope`, `:ontology`, and `:ingestion`; replaceable Lenses use `:name`, `:scope`, `write: :replace_graph`, and `:graph_format`. `JidoGralkor.Plugin` selects registered names through `:ingestion_lens` and `:search_lenses`; the removed `:default_lens` option raises with migration guidance. See README `Configure Lenses` for the consumer API. Registry and mount validation fail fast. Deployment-wide `:ontology` applies only to the implicit-operator compatibility path.

**Reflections.** `:reflections` is a list of `name:`, `scope:`, and `chain_of_thought:` definitions. Omitting it loads packaged `generalisations` and `erl`; supplying it replaces those defaults. `:reflection_root` defaults to `Application.app_dir(:jido_gralkor)` and resolves relative YAML paths; `:reflection_storage` defaults to `Gralkor.Reflection.Storage.Graphiti`. Invalid names, duplicates, scopes, paths, YAML steps, interpolations, or output schemas fail validation before processing. See README `Configure Reflections`.
...
**Implicit-operator auto-generalise on flush (optional).** Set `:jido_gralkor, :generalise_on_flush` to `true` to fire legacy `Gralkor.Generalise` after each successful implicit-operator capture flush. Registered Lens mounts use `generalise_lens` instead. `:generalise_min_confidence` (default `0.3`) applies to both generalisation pipelines.
...
**Legacy `_gen` generalisation.** `Gralkor.Generalise` writes each kept generalisation as a *new* episode with no `uuid`. Graphiti's `add_episode(uuid: …)` **loads an existing** episode to re-extract against, so passing a freshly minted id raised `NodeNotFoundError` and every generalisation write failed silently inside the flush task. A generalisation's id now lives only in the episode body, where it records lineage between generalisations; nothing deletes an earlier generalisation, which matches `Gralkor.Lens.Ingestion.Generalise`. Retrieval splits by what each caller needs: recall's generalisation search reads **nodes** (`GraphitiPool.search_nodes`) because it wants semantically relevant memory, while `Client.search_generalisations/3` reads **episodes** (`GraphitiPool.search_episodes/4`, graphiti's BM25-over-content search) because it must decode the `GEN|v1|` envelope, which only the stored body carries. Neither reads edges any more — the same lesson as ERL, one step further: an episode naming a single subject yields a node and no edge, on a given run may yield neither, and no derived fact ever carries the wire prefix, so the old decode-the-edge-fact path returned nothing on every call.
