...
- **Destinations** — each Destination resolves to one logical graph ID. `global` is shared, `personal` resolves to `personal/<agent.id>`, and an application Destination resolves to its exact name; application names under the reserved `personal/` and `operator/` namespaces are rejected. Graphiti encodes that logical ID exactly once as `g_` plus lowercase hexadecimal bytes. Definitions contain only `:name`; Lenses and Reflection Destination outputs reference Destinations and select the ontology for their writes. Search defaults to episodes across every accessible registered Destination, accepts optional per-call Destination and Lens selectors, and retains explicit facts, nodes, and artefacts as advanced result types. See [`DESTINATIONS.md`](DESTINATIONS.md).
...
- **Reflections** — runtime configuration declares named synthesis processes with an inline structured Chain of Thought and exactly one Destination output. `Gralkor.Client.reflect/5` admits work through the consuming agent and immediately returns the invocation ID; the agent runtime independently produces and delivers the artefact, retries 5xx failures exponentially for up to twenty-four hours, abandons 4xx failures immediately, and invokes the per-submission callback with terminal production or delivery outcomes reached during its lifetime. Runtime termination cancels unfinished work without invoking its callback. The Reflection Runner owns step ordering and exact output validation while Jido AI's standalone ReAct runtime executes each step's model and tool loop; prompts are governed by the model context limit, with ten iterations per step. `%Gralkor.Artefact{}` contains exactly `id` and `payload`; `generalisations` and `erl` are always-installed packaged definitions.
...
- **Explicit capture** — every mount supplies `capture_destination`; absent `ingestion_lens` selects direct storage, while an explicitly selected `personal-chat` or consumer Lens runs its own process without an extra direct write.
- **`JidoGralkor.Plugin`** — owns the `:__memory__` slot, validates `agent_name`, explicit `capture_destination`, and complete runtime configuration, and starts its linked runtime child. It captures through `Client.capture/2` with a typed direct or Lens route. A per-turn binary `lens` selects that Lens; explicit nil selects direct capture. Request refs preserve that selection for completion and failure. Missing identity or capture failures surface immediately; missing committed threads skip capture with a warning.
...
- **`JidoGralkor.Actions.MemoryAdd`** — the fire-and-forget write tool. Its background task calls the runtime-targeted `Gralkor.Client.ingest/2` when context selects a Lens and uses `memory_add/3` against the graph `personal/<agent.id>` otherwise; failures are logged and the action immediately returns `"Ingesting."`.
...
**Embedded Gralkor adapter.** `Gralkor.Capture` separates session/operator/name/message fields from `{:direct, destination}` or `{:lenses, names}`. Capture validates and snapshots route definitions before buffering, binds session identity, and independently flushes compatible direct or Lens batches in turn order. Positional capture adapters raise migration guidance. Ingestion IDs survive retries, but uncertain writes can duplicate and do not promise exactly-once delivery. Capture and Lens ingestion never trigger Reflections. Direct writes carry storage-owned direct provenance; real and historical Lens markers remain unchanged. `_gralkor_lens` continues to mean replacement ownership.
...
```bash
mix test            # Unit + Integration; excludes Functional and Journey; embedded boundary tests use real Python/Graphiti with substituted inference
...
mix test.journey    # one broad production-like memory lifecycle with real configured services
