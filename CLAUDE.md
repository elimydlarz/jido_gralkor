...
**Reflections.** `:reflections` is a list of `name:`, `destination:`, and `chain_of_thought:` definitions. Omitting it loads packaged `generalisations` and `erl`; supplying it replaces those defaults. `:reflection_root` defaults to `Application.app_dir(:jido_gralkor)` and resolves relative YAML paths; `:reflection_storage` defaults to `Gralkor.Reflection.Storage.Graphiti`; custom stores must implement deterministic create-or-confirm and incomplete-artefact lookup semantics documented by `Gralkor.Reflection.Store`. `:reflection_scheduler_journal_path` overrides the default DETS path and must be unique per BEAM node when nodes share a filesystem. Invalid names, Destinations, paths, YAML steps, interpolations, output schemas, retry delays, or execution timeouts fail validation before processing. Scheduler identity is `{operator_id, ingestion_id, reflection_name}`; lookup, Runner, and storage retry independently with one active attempt, retry deadlines survive restart, crash-interrupted work consumes the durable retry budget, an uncertain active storage attempt is first confirmed, terminal failure is logged, and the exact deterministic artefact is retained across storage retries. Drain closes admission after all tracked capture flush workers finish. Graphiti equal repeats confirm without extraction only after a durable extraction-completion marker; an equal partial episode is hidden from artefact search and resumes extraction without rerunning the Runner, while conflicting immutable content is terminal. Runner and host-tool effects remain at-least-once.
