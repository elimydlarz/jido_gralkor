...
- **Reflections** — `config :jido_gralkor, :reflections` declares named asynchronous post-ingestion processes, each referencing a Destination and repository YAML Chain of Thought. `generalisations` and `erl` are packaged defaults; the packaged `erl` Reflection carries `Gralkor.Reflection.ERLOntology` and its `Learning` entity.
