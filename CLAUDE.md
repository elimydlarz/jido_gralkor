...
`JidoGralkor.Lifecycle`'s sole job is now the death-triggered flush: when an agent process terminates, fire `Gralkor.Client.flush/1` for the active session so the buffered turns don't strand in `Gralkor.CaptureBuffer` state outliving the agent. The idle-timer machinery is no longer part of this module — Jido's `AgentServer` owns idle timing via its built-in `:idle_timeout` option, and consumers that don't want a reaper just don't pass it (default is `:infinity`). Memory consolidation during a live agent's lifetime is owned by `JidoGralkor.ContextRotator` (see below), not by death.
...
`JidoGralkor.ContextRotator` is a plain module exposing `rotate_now/2` (synchronous): given a pid for an alive Jido AgentServer, it flushes the active Gralkor session, installs a fresh thread on the agent (so the next turn runs under a new session id), and seeds the rotated thread with two slices:

1. The most-recent `:keep_last_n` of the entries that existed BEFORE the flush — short-range conversational continuity for what was just moved to long-term memory.
2. Any thread entries appended DURING the flush — in-flight turns that weren't part of the just-flushed batch and must not be dropped.

Pre-flush entries outside the `:keep_last_n` tail are removed from the in-memory context (they're in long-term memory; recall is the path back). The agent process is never stopped.

Periodic rotation is left to the consumer (a small wrapper GenServer or a sensor). This module deliberately owns no timer — its single responsibility is the rotation primitive itself, callable from `/new` chat commands and from any scheduling mechanism.
