...
JidoGralkor.Plugin
  ...
  when an agent turn begins
    ...
    when recall returns nothing
      when a thread had committed before recall
        then the signal passes through with the session_id stashed on tool_context; `:query` is unchanged
      when no thread had committed before recall
        then the signal passes through unchanged (no session_id stashed); `:query` is unchanged
    if recall fails
