...
**Known-failing on the OpenAI pin — [`FAILURES.md`](FAILURES.md) is the bug report.** Journey modules must be judged **one at a time** (`jido_memory_journey` 2/3, `generalise_journey` 1/2); a whole-suite `mix test.journey` reports 3/7 for a reason that is an artefact of running them together, not a defect in the code under test.
