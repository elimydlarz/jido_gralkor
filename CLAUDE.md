...
`test/functional/interpret_epistemic_humility_test.exs` is the focused real-model proof for interpretation behavior. It loads `OPENAI_API_KEY` from `.env`, calls OpenAI `gpt-4.1` through ReqLLM at temperature `0.0` (`gpt-4.1-mini` resolved the adversarial conflicting-accounts case into one asserted fact about one run in four, even with the instruction sharpened). Read a green run for what it is: the instruction in `Gralkor.Interpret.epistemic_instruction/0` is production English whose behaviour only a model can reveal, and a capable model complying with it does not establish that conflict preservation holds on a weak one — no model other than these two has been measured, while consumers recall on whatever they configure, and covers varied source types, adversarial conflicting accounts, ordinary memories without provenance, and relevance filtering. It starts no Graphiti or FalkorDB runtime. Missing credentials fail fast rather than skip.
