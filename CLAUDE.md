...
`.env.example` pins `GRALKOR_LLM_MODEL=openai:gpt-4.1` and `GRALKOR_EMBEDDER_MODEL=openai:text-embedding-3-small`, so every suite that reaches a real provider runs on `OPENAI_API_KEY` alone; the package defaults remain Google, and `GOOGLE_API_KEY` is needed only when a role is pointed back at Google. The four LLM-backed suites (`ontology_extraction`, `generalise_journey`, `jido_memory_journey`, `remote_falkordb_journey`) build their `GraphitiPool` child spec from `Gralkor.Config.llm_model/0` and `embedder_model/0` rather than naming a provider, so they follow whatever those variables select.

`gpt-4.1-mini` is too weak for the ontology-extraction assertions — it produced `User` but no `Preference` node from the fixture. The no-ontology case (`assert at least one generic Entity`) is **flaky on any model**: observed fail/fail/pass across three identical `gpt-4.1` runs, generic extraction being free to return nothing. A failure there is LLM variance, not a regression.
