...
`.env.example` pins `GRALKOR_LLM_MODEL=openai:gpt-4.1-mini` and `GRALKOR_EMBEDDER_MODEL=openai:text-embedding-3-small`, so every suite that reaches a real provider runs on `OPENAI_API_KEY` alone; the package defaults remain Google, and `GOOGLE_API_KEY` is needed only when a role is pointed back at Google.
