...
**Lenses.** `:lenses` is a list of application-owned keyword definitions. Appending Lenses use `:name`, `:scope`, `:ontology`, and `:ingestion`; replaceable Lenses use `:name`, `:scope`, `write: :replace_graph`, and `:graph_format`. `JidoGralkor.Plugin` selects registered names through `:ingestion_lens` and `:search_lenses`; the removed `:default_lens` option raises with migration guidance. See README `Configure Lenses` for the consumer API. Registry and mount validation fail fast. Application-owned ontologies belong only to registered named Lenses.
...
**Implicit-default ontology.** Jido Gralkor owns `Gralkor.DefaultOntology`, the open generic extraction contract used by legacy capture, `memory_add/3`, recall's reserved operator channel, and the implicit `"operator"` Lens. There is no deployment-wide `:ontology` setting and no `memory_add/4` override. A custom extraction schema must be attached to an explicitly registered named Lens.
