  ...
  orchestration
    when called
      ...
      then a learning search runs unconditionally on every implicit-default recall (§1 ERL recall — no flag, no LLM classification; Lens searches use Client.search/1 instead)
        ...
        where the learning search uses graphiti NODE search (GraphitiPool.search_nodes), not edge search — a Learning is a custom-entity NODE, and edge search's node_labels filter matches edges by endpoint, so it would miss standalone Learning nodes and silently return [] (integration: real Native.recall/4 → real learning_search_fn → real GraphitiPool.search_nodes; fake graphiti returns no nodes so the LLM interpret_fn is never called; assert no learning-search-failed log and node_labels: ["Learning"] reached g.search_; test/gralkor/recall_test.exs)
...
ex-capture-buffer (src: lib/gralkor/capture_buffer.ex; unit: test/gralkor/capture_buffer_test.exs)
  ...
  append/6 (session_id, group_id, agent_name, user_name, ontology, messages)
    ...
    if user_name is missing or blank
      then raises ArgumentError
  append_lens/6 (session_id, operator_id, agent_name, user_name, lens, messages)
...
ex-python-runtime (src: lib/gralkor/python.ex; unit: test/gralkor/python_test.exs)
  Gralkor.Python's init/1 runs the boot sequence synchronously and returns only when ready
    ...
    then the uv-managed venv is materialised if absent and PythonX is initialised pointing at it, via `Pythonx.uv_init/2` from `priv/python/pyproject.toml` (read into `@pyproject_toml` at compile time via `@external_resource`, so editing the manifest triggers recompilation)
      (graphiti-core + falkordblite + provider deps installed. Because `Pythonx.uv_init/2` runs with `priv?: false`, the venv lands in PythonX's uv cache — `$PYTHONX_CACHE_DIR` or the OS user-cache dir — under `projects/<md5-of-pyproject>`, NOT in the app's `priv/`. It is therefore built at runtime on first boot rather than baked into a release; changing the manifest yields a fresh project dir. This is what makes the embedded Python stack self-contained: a consumer like susu configures nothing about Python — there is no `:pythonx, :uv_init` block in any consumer config — because the graphiti-core version requirement lives in jido_gralkor's own packaged manifest (shipped via the `priv` entry in `mix.exs` `files:`). Idempotent within a VM: a `{Gralkor.Python, :uv_inited}` flag in `:persistent_term` short-circuits the second call so multiple boots — as functional-test modules do — never hit the PythonX NIF's "already been initialized" guard.)
...
ex-graphiti-pool (src: lib/gralkor/graphiti_pool.ex; unit: test/gralkor/graphiti_pool_test.exs; integration: test/gralkor/graphiti_pool_test.exs)
  ...
  Gralkor.GraphitiPool's init/1 runs synchronously
    ...
    when started with an embedded spec (`{:embedded, data_dir: dir}`)
      ...
      then the embedded FalkorDB construction boundary receives the data directory once and the resulting database is held for the lifetime of the GenServer
      (functional memory journeys exercise the default redislite-backed `<data_dir>/gralkor.db` constructor)
...
JidoGralkor.Plugin (src: lib/jido_gralkor/plugin.ex; unit: test/jido_gralkor/plugin_test.exs)
  registers MemorySearch, MemoryAdd, MemoryBuildIndices, and MemoryBuildCommunities as plugin actions, exposed via the autogenerated `actions/0` for consumers to pass to `tools:`
  a consumer agent that mounts the plugin compiles (no Jido route conflicts)
  the session_id is the Jido thread id — the plugin does not mint its own
  mount/2 returns {:ok, %{agent_name: ...}} when given a non-blank :agent_name opt
  mount/2 raises ArgumentError when :agent_name is missing
  mount/2 raises ArgumentError when :agent_name is blank
  ...
  when an agent turn begins, when a thread has committed to agent state
    the thread's session_id and agent_name are planted on the signal's tool_context
    the plugin does not call recall on its own (recall is the LLM's job)
    the query is left untouched on the signal
  when an agent turn begins, when the plugin is Lens-aware
    then the selected Lens and search targets are planted with the agent name and optional session id
    then the selected Lens and search targets are planted without a session id when no thread has committed
  when an agent turn begins, when the plugin uses implicit-default and no thread has committed yet
    only the configured agent_name is planted on tool_context (no session_id)
    the plugin still does not call recall
  when an agent turn completes
    the turn is sent to Gralkor for capture as canonical messages with the thread's session_id and the operator's group_id
  ...
  when an agent turn fails
    the turn is captured with the failure surfaced as a terminal 'request failed: …' behaviour message
    first-turn failure with no thread committed skips capture and logs a warning
  when a signal of any other type arrives
    handle_signal/2 returns {:ok, :continue} with no captures and no recalls
  ...
  ERL round-trip
    given a captured turn (capture/5) whose reasoning solved a problem
      when Gralkor.Client.flush/1 is called and the flush callback is wired with learn_fn
        then the learning is written via add_episode with the Learning custom entity type, and graphiti's extractor creates a Learning-labelled node with populated attributes (problem_kind/approach/success/lesson)
        and a NODE search (GraphitiPool.search_nodes with node_labels: ["Learning"]) keyed on the kind of problem returns that Learning node carrying the lesson — proving the entity was extracted and node-label filtering retrieves it; isolates the ERL recall path from the unfiltered main search, which shares the partition and would surface the episode regardless
