  ...
  orchestration
    when called
      ...
      then a learning search runs unconditionally on every recall (§1 ERL recall — no flag, no LLM classification)
        ...
        where the client-wired learning_search_fn calls GraphitiPool.search with the search_filter passed as the opts keyword list (5th arg, server explicit), not as a positional arg — search/5 has defaults on server (1st) and opts (5th), so a 4-arg call binds the kwarg to max_results and fails the guard; the task would raise FunctionClauseError, await_aux swallows it and logs "[gralkor] recall learning search failed: {:exit, ...}", and ERL silently degrades to [] on every recall (integration: real Native.recall/4 → real learning_search_fn closure → real GraphitiPool.search with SearchFilters; fake graphiti returns [] so the LLM interpret_fn is never called; assert no learning-search-failed log and has_filter reached graphiti; test/gralkor/recall_test.exs)
