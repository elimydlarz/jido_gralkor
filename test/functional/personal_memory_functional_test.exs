defmodule Gralkor.PersonalMemoryFunctionalTest do
  ...
  setup do
    ...
    on_exit(fn -> Enum.each(previous, fn {key, value} ->
      if value, do: Application.put_env(:jido_gralkor, key, value), else: Application.delete_env(:jido_gralkor, key)
    end) end)
  ...
  describe "when an application captures directly to the registered personal Destination without selecting a Lens" do
    ...
    test "and no packaged Lens ingestion process runs", context do
      capture({:direct, "personal"})
      assert [%{"source_description" => "captured [gralkor: direct]"}] = recorded(context.graphiti)
    end
  end

  describe "when an application selects the packaged personal-chat Lens" do
    test "then its Store ingestion process writes to that operator's personal Destination", context do
    ...
    test "and its built-in ontology applies to conversation and other supported source kinds" do
      for {kind, content} <- [{:conversation, "Eli: hello"}, {:document, "hello"}, {:structured_record, %{hello: true}}] do
        assert :ok = Client.ingest(self(), %Gralkor.Ingest{id: "kind-#{kind}", operator_id: "owner", lens: "personal-chat", source_kind: kind, content: content, source_description: "source"})
      end
      assert JidoGralkor.Runtime.lens!(self(), "personal-chat").ontology == Gralkor.DefaultOntology
    end
    ...
    test "and no additional direct capture write occurs", context do
      capture({:lenses, ["personal-chat"]})
      assert [%{"source_description" => "captured [lens: personal-chat]"}] = recorded(context.graphiti)
    end
  end
  ...
  describe "if an application selects the retired operator Lens or Destination" do
    test "then the request fails with an explicit migration error before capture or graph access", context do
  ...
  defp capture(route) do
    assert :ok = Client.capture(self(), %Gralkor.Capture{session_id: "personal-session", operator_id: "owner", agent_name: "Susu", user_name: "Eli", messages: [Gralkor.Message.new("user", "Remember teal")], route: route})
    Client.impl().flush_and_await("personal-session", 5_000)
  end

  defp search, do: Client.search(self(), %Search{operator_id: "owner", query: "teal", destinations: ["personal"]})

  defp recorded(graphiti) do
    {raw, _} = Pythonx.eval("[{ 'group_id': item['group_id'], 'source_description': item['source_description'], 'has_entity_types': 'entity_types' in item } for item in g.recorded]", %{"g" => graphiti})
    Pythonx.decode(raw)
  end
end
