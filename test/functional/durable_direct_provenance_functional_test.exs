defmodule Gralkor.DurableDirectProvenanceFunctionalTest do
  ...
  describe "when direct memory is stored in a real graph" do
    test "then ordinary direct writes retain durable writer metadata through public episode and fact search",
         context do
      assert_direct_round_trip(context, false)
    end

    test "and deterministic direct writes retain durable writer metadata through public episode and fact search",
         context do
      assert_direct_round_trip(context, true)
    end

    test "and historical marker-like text without durable metadata stays unchanged and unclassified",
         context do
