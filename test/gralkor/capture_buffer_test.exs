defmodule Gralkor.CaptureBufferTest do
  ...
  describe "when a typed capture request supplies resolved direct and Lens routes" do
    test "then each route buffers only its selected turns while session reads retain append order" do
      ...
      assert_receive {:lens_flushed, "owner", "Susu", "Eli", ^lens, lens_turns}
      assert lens_turns == [second.messages]
    end
    ...
    test "and a session rejects changes to its runtime owner, operator, agent, or user before buffering" do
  ...
  describe "if typed capture and compatibility buffer calls reuse one session" do
    test "then the second mode is rejected without changing accepted turns" do
      for mode <- [:direct, :lens], order <- [:typed_first, :compatibility_first] do
        ...
        request = %{typed_request("original") | session_id: session, messages: messages}
        ...
        capture = fn ->
          CaptureBuffer.append_capture(self(), request, [
            {:direct, "personal/owner", Gralkor.DefaultOntology}
          ])
        end
