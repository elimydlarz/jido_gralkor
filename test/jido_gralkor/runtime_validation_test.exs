defmodule JidoGralkor.RuntimeValidationTest do
  ...
  describe "top-level and collection validation" do
    test "identifies every unknown top-level field" do
      assert {:error, {:unknown_configuration_fields, [:extra, :other]}} =
               validate(
                 Map.merge(valid_configuration(:destinations, "memory"), %{extra: 1, other: 2})
               )
    end
    ...
    for collection <- [:destinations, :lenses, :reflections] do
      test "identifies missing #{collection} collection" do
        assert {:error, {:missing_collection, unquote(collection)}} =
                 validate(
                   Map.delete(valid_configuration(:destinations, "memory"), unquote(collection))
                 )
      end
      ...
      test "identifies invalid #{collection} collection" do
        assert {:error, {:invalid_collection, unquote(collection), :bad}} =
                 validate(
                   Map.put(
                     valid_configuration(:destinations, "memory"),
                     unquote(collection),
                     :bad
                   )
                 )
    ...
    test "identifies a definition with the collection and configured value" do
      assert {:error, {:invalid_definition, :destinations, :bad}} =
               validate(
                 Map.put(valid_configuration(:destinations, "memory"), :destinations, [:bad])
               )
    end
    ...
    test "identifies unknown definition fields" do
      configuration =
        update_in(
          valid_configuration(:destinations, "memory"),
          [:destinations, Access.at(0)],
          &Keyword.put(&1, :extra, true)
        )
  ...
  describe "reserved names and destination references" do
    test "identifies a package-owned name" do
      assert {:error, {:reserved_definition_name, :destinations, "operator"}} =
               validate(
                 Map.put(valid_configuration(:destinations, "memory"), :destinations, [
                   [name: "operator"]
                 ])
               )
    end
    ...
    test "identifies the retired default Lens name" do
      assert {:error, {:retired_definition_name, :lenses, "default", "operator"}} =
               validate(
                 Map.put(valid_configuration(:destinations, "memory"), :lenses, [
                   [
                     name: "default",
                     destination: "memory",
                     write: :append,
    ...
    test "identifies an unknown Lens Destination" do
      configuration =
        update_in(
          valid_configuration(:destinations, "memory"),
          [:lenses, Access.at(0)],
          &Keyword.put(&1, :destination, "missing")
        )

      assert {:error, {:unknown_destination, :lenses, "notes", "missing"}} =
               validate(configuration)
    end
  end
  ...
  describe "Lens shape validation" do
    ...
    test "rejects invalid ingestion, ontology, incompatible fields, and write mode" do
      ...
      bad_ingestion =
        update_in(base, [:lenses, Access.at(0)], &Keyword.put(&1, :ingestion, String))
      ...
      replace =
        update_in(base, [:lenses, Access.at(0)], fn lens ->
          lens
          |> Keyword.put(:write, :replace_graph)
          |> Keyword.delete(:ontology)
          |> Keyword.delete(:ingestion)
        end)
      ...
      incompatible =
        update_in(
          replace,
          [:lenses, Access.at(0)],
          &Keyword.put(&1, :ontology, Gralkor.DefaultOntology)
        )
  ...
  describe "Reflection output and Chain of Thought validation" do
    test "identifies invalid outputs and output counts" do
      ...
      assert {:error, {:invalid_reflection_outputs, "review", :bad}} =
               validate(
                 update_in(base, [:reflections, Access.at(0)], &Keyword.put(&1, :outputs, :bad))
               )
    ...
    test "identifies invalid output fields and Chain of Thought declarations" do
      ...
      assert {:error, {:missing_reflection_destination, "review", nil}} =
      ...
      assert {:error, {:unknown_destination, :reflections, "review", "missing"}} =
               validate(unknown_destination)

      missing_cot =
        update_in(base, [:reflections, Access.at(0)], &Keyword.delete(&1, :chain_of_thought))

      assert {:error, {:missing_chain_of_thought, "review"}} = validate(missing_cot)

      bad_cot =
        update_in(base, [:reflections, Access.at(0)], &Keyword.put(&1, :chain_of_thought, :bad))
      ...
      assert {:error, {:unknown_chain_of_thought_fields, "review", [:extra]}} =
               validate(unknown_cot)
    end
  end
