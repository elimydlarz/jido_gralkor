defmodule JidoGralkor.RuntimeValidationTest do
  ...
  describe "if a Destination, Lens, or Reflection name is missing, blank, or duplicated" do
    test "then validation identifies the collection and invalid name" do
      for collection <- [:destinations, :lenses, :reflections], value <- [nil, false, "", "  "] do
        assert {:error, {:blank_definition_name, ^collection, ^value}} = validate(named(collection, value))
      end

      for collection <- [:destinations, :lenses, :reflections] do
        assert {:error, {:duplicate_definition_name, ^collection, "same"}} = validate(named(collection, "same", true))
      end
    end
  end
  ...
  describe "if runtime configuration contains an unknown top-level field" do
    test "then validation identifies every unknown field" do
      assert {:error, {:unknown_configuration_fields, [:extra, :other]}} = validate(Map.merge(config(), %{extra: 1, other: 2}))
    end
  end
  ...
  describe "if a required Destination, Lens, or Reflection collection is absent" do
    test "then validation identifies the missing collection" do
      for collection <- [:destinations, :lenses, :reflections] do
        assert {:error, {:missing_collection, ^collection}} = validate(Map.delete(config(), collection))
      end
    end
  end
  ...
  describe "if a Destination, Lens, or Reflection collection is not a list" do
    test "then validation identifies the collection and configured value" do
      for collection <- [:destinations, :lenses, :reflections] do
        assert {:error, {:invalid_collection, ^collection, :bad}} = validate(Map.put(config(), collection, :bad))
      end
    end
  end
  ...
  describe "if a definition is neither a map nor a keyword list" do
    test "then validation identifies its collection and configured value" do
      assert {:error, {:invalid_definition, :destinations, :bad}} = validate(Map.put(config(), :destinations, [:bad]))
    end
  end
  ...
  describe "if a definition contains unknown fields" do
    test "then validation identifies its collection, name, and unknown fields" do
      ...
      assert {:error, {:unknown_definition_fields, :destinations, "memory", [:extra]}} = validate(c)
    end
  end
  ...
  describe "if a consumer definition uses a name reserved by a package-owned definition" do
    test "then validation identifies its collection and reserved name" do
      assert {:error, {:reserved_definition_name, :destinations, "operator"}} = validate(Map.put(config(), :destinations, [[name: "operator"]]))
    end
  end
  ...
  describe "if a Destination name uses the reserved `operator/` namespace or a Lens uses the retired `default` name" do
    test "then validation identifies the reserved or retired name" do
      assert {:error, {:reserved_destination_namespace, "operator/custom"}} = validate(Map.put(config(), :destinations, [[name: "operator/custom"]]))
      lens = [[name: "default", destination: "memory", write: :append, ingestion: Gralkor.Lens.Ingestion.Store, ontology: Gralkor.DefaultOntology]]
      assert {:error, {:retired_definition_name, :lenses, "default", "operator"}} = validate(Map.put(config(), :lenses, lens))
    end
  end
  ...
  describe "if a Lens or Reflection name contains the reserved provenance delimiter ` [lens: `" do
    test "then validation identifies the collection and name" do
      lens = [[name: "notes [lens: old", destination: "memory", write: :append, ingestion: Gralkor.Lens.Ingestion.Store, ontology: Gralkor.DefaultOntology]]
      assert {:error, {:reserved_provenance_syntax, :lenses, "notes [lens: old"}} = validate(Map.put(config(), :lenses, lens))
    end
  end
  ...
  describe "if a map supplies an atom-keyed ontology value and a string-keyed ontology value" do
    test "then the atom-keyed value remains authoritative even when it is false" do
      c = update_in(config(), [:lenses, Access.at(0)], &Keyword.put(&1, :ontology, false))
  ...
  describe "when a replaceable Lens declares `write: :replace_graph` and a Destination" do
    test "then it resolves as a replaceable Lens" do
      ...
      assert :ok = validate(Map.put(config(), :lenses, lens))
    end
  end
  ...
  describe "if a replaceable Lens also declares ingestion or ontology fields" do
    test "then validation identifies the incompatible Lens definition" do
      lens = [[name: "notes", destination: "memory", write: :replace_graph, ontology: Gralkor.DefaultOntology]]
      assert {:error, {:incompatible_lens_definition, "notes"}} = validate(Map.put(config(), :lenses, lens))
    end
  end
  ...
  describe "if a Reflection declares no Destination output or more than one Destination output" do
    test "then validation identifies the Reflection and output count failure" do
      ...
      outputs = [[kind: :destination, destination: "memory", ontology: Gralkor.DefaultOntology], [kind: :destination, destination: "memory", ontology: Gralkor.DefaultOntology]]
  ...
  describe "if a Reflection declares an unsupported output kind" do
    test "then validation identifies the Reflection and output kind" do
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :outputs, [[kind: :other]]))
  ...
  describe "if a Reflection output has a missing or unknown Destination or invalid ontology" do
    test "then validation identifies the Reflection and invalid output field" do
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :outputs, [[kind: :destination, ontology: Gralkor.DefaultOntology]]))
      assert {:error, {:missing_reflection_destination, "review", nil}} = validate(c)
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :outputs, [[kind: :destination, destination: "missing", ontology: Gralkor.DefaultOntology]]))
      assert {:error, {:unknown_destination, :reflections, "review", "missing"}} = validate(c)
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :outputs, [[kind: :destination, destination: "memory", ontology: String]]))
  ...
  describe "if a Reflection's Chain of Thought is missing, unstructured, empty, or contains a malformed step" do
    test "then validation identifies the Reflection and Chain of Thought failure" do
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.delete(&1, :chain_of_thought))
      assert {:error, {:missing_chain_of_thought, "review"}} = validate(c)
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :chain_of_thought, :bad))
      assert {:error, {:invalid_chain_of_thought, "review", :bad}} = validate(c)
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :chain_of_thought, [steps: []]))
      assert {:error, {:invalid_chain_of_thought, "review", :missing_steps}} = validate(c)
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :chain_of_thought, [steps: [:bad]]))
  ...
  describe "if a Chain of Thought or one of its steps contains unknown fields" do
    test "then validation identifies the Reflection, step when applicable, and unknown fields" do
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :chain_of_thought, [steps: [], extra: true]))
      assert {:error, {:unknown_chain_of_thought_fields, "review", [:extra]}} = validate(c)
      c = update_in(config(), [:reflections, Access.at(0)], &Keyword.put(&1, :chain_of_thought, [steps: [[label: "inspect", directions: "Inspect.", output: %{"summary" => "string"}, extra: true]]]))
      assert {:error, {:unknown_chain_of_thought_step_fields, "review", "inspect", [:extra]}} = validate(c)
    end
  end

  defp validate(configuration), do: Runtime.validate(configuration, packaged_reflections: fn -> [] end, parse_chain_of_thought: &parse_chain_of_thought/1)

  defp config do
    %{destinations: [[name: "memory"]], lenses: [[name: "notes", destination: "memory", write: :append, ingestion: Gralkor.Lens.Ingestion.Store, ontology: Gralkor.DefaultOntology]], reflections: [[name: "review", outputs: [[kind: :destination, destination: "memory", ontology: Gralkor.DefaultOntology]], chain_of_thought: [steps: [[label: "inspect", directions: "Inspect.", output: %{"summary" => "string"}]]]]]}
  end
  ...
  defp parse_chain_of_thought(%{steps: [%{label: "inspect", directions: "Inspect.", output: %{"summary" => "string"}}]}) do
    {:ok, %Gralkor.Reflection.ChainOfThought{steps: [%Gralkor.Reflection.ChainOfThought.Step{label: "inspect", directions: "Inspect.", output: %{"summary" => "string"}}]}}
  end
end
