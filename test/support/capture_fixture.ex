defmodule Gralkor.CaptureFixture do
  ...
  @spec capture(module(), String.t(), String.t(), String.t(), String.t(), [Gralkor.Message.t()]) ::
          term()
  ...
  def capture_arguments(adapter, arguments),
    do: apply(__MODULE__, :capture, [adapter | arguments])
  ...
  defp request(adapter, session, operator, agent, user, messages, route) do
        ...
        ExUnit.Callbacks.start_supervised!(
          {JidoGralkor.Runtime,
           ...
           configuration: %{
             destinations:
               Enum.map(
                 ["g", "g1", "g-1", "with-hyphens", "group-1", "operator_one"],
                 &%{name: &1}
               ),
             lenses:
               Enum.map(
                 ["observations", "generalisations", "decisions"],
                 &%{
                   name: &1,
    ...
    adapter.capture(self(), %Gralkor.Capture{
      session_id: session,
