...
- What graphiti extracts from an episode is not knowable in advance: a statement naming one subject yields a node and no edge. Anything Gralkor must find again is therefore retrieved by node search as well as edge search, and a custom entity type carries a description because that is what the extractor reads to decide when to mint it.
