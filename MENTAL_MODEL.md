...
- *fact* — the text of one edge an edge search returned; recall presents facts verbatim in search order, retaining their available source wording for the consuming agent to interpret.
...
- Public search validates every selected Destination, result type, and positive result limit before any query begins, defaults an empty selection to packaged operator memory and global generalisations, and searches distinct Destinations concurrently. Results retain requested Destination order and identify their Destination; entity and edge type filters apply to nodes and facts respectively. Embedded episode writes are admitted one at a time through their shared local connection without admitting searches; remote writes remain concurrent.
...
- Recalled graph content is memory context, not adjudicated truth. Gralkor preserves the search result's wording, order, and available source context without filtering or rewriting it through another model; interpretation and truth-sensitive verification belong to the consuming agent, which can use its own model and conversation context.
