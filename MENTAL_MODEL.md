...
- The embedded Python stack is a consumer-invisible internal concern: `Gralkor.Python.init/1` materialises the venv via `Pythonx.uv_init/2` from a jido_gralkor-owned pyproject (the graphiti-core pin), so consumers configure *nothing* about Python. The pin must live in code rather than `config/config.exs` because a dep's config does not propagate to a consumer's runtime app env and Pythonx reads its pin via `Application.compile_env` — leaving it in config forced every consumer to restate (and silently drift on) the version.
