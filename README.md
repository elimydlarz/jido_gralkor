...
Before packaged generalisation inference begins, Gralkor performs one episode search across `operator`, `global`, and every Destination referenced by the completed representations' Lenses. Its query contains every current representation, so the same search returns related ingested information and existing generalisation artefacts from `global`; inference receives those results separately from the current representations. A related-memory search failure fails that Reflection before inference without changing the completed ingestion.

Each stored generalisation contains exactly `content`, `level`, and `generalises_over`. `generalises_over` contains only the influencing prior generalisations, each snapshotted as exactly its `content` and `level`. A generalisation with none is level `1`; otherwise its level is one greater than the highest referenced level.
