...
**Structured memory search.** `Gralkor.Client.search/1,2` returns a list of attributed records, and `memory_search` returns structured results in `{:ok, %{result: results, omissions: %{byte_budget: count}}}`. The action retains whole results within `tool_context[:memory_search_max_bytes]` (65,536 bytes by default), accounting for the complete serialized envelope. Lens episodes retain textual `content`, `source_description`, and `lens`. Reflection episodes expose `artefact: %{id: id, payload: payload}`, `source_description`, and `reflection`; consumers no longer decode JSON from `episode.content`. Fact searches return fact maps with timestamps and source attribution; `Gralkor.Format.format_fact/1` is the explicit readable formatter. See [the result schema and consumer migration](DESTINATIONS.md#consumer-migration-and-model-delivery), including the unchanged blank-query NON-RESULT, required lossless Jido AI transport change, and current dependency installation status.
