...
when the deployment configures an inference LLM and an embedder
  ...
  while the configured LLM provider and the configured embedder provider differ
    then each role's client is built for its own provider
    and the memory runtime starts
...
where the deployment configures no LLM or embedder override
  ...
  and only the Google credential is required for the memory runtime to start

while the deployment has opted into the native memory runtime
  if a configured provider is neither OpenAI nor Google
    then the memory runtime refuses to start before any inference client is constructed
    and the failure names both configured model specs
    and the failure names the providers that are supported
  if the credential for a configured provider is absent or blank
    then the memory runtime refuses to start before any inference client is constructed
  ...
  where a provider is named by neither role's configuration
    then its absent credential does not prevent the memory runtime from starting

while the deployment has not opted into the native memory runtime
  where a configured provider is unsupported or its credential is absent
    then startup is unaffected, because no inference client is constructed at all
