Functional: retry-ownership (src: lib/gralkor/capture_buffer.ex, lib/gralkor/client.ex, lib/gralkor/reflection/scheduler.ex, lib/gralkor/destination/storage.ex; functional: test/functional/retry_ownership_functional_test.exs)
...
when a Reflection Runner or Destination output write fails after completed Lens ingestion
  then the Reflection Scheduler alone owns its bounded retry schedule
  and the Runner and Destination store each make one attempt when invoked by the Scheduler
