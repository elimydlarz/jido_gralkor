...
when an application registers a valid appending or replaceable Lens
  ...
  and the Lens owns an inline Destination whose name is derived from the Lens name

where an appending Lens definition provides a Destination and ingestion process without a write mode
...
if an existing Lens definition retains top-level scope or ontology settings
  then configuration resolution raises `ArgumentError` identifying the inline Destination migration
...
if an application registers an invalid Lens
  ...
  and an invalid Lens Destination is identified with its Lens
