...
when the Python runtime is initialised
  ...
  and the managed virtual environment is materialised if absent and the interpreter is pointed at it
...
when the Python runtime is initialised while an embedded connection is configured
...
when the Python runtime is initialised while a remote connection is configured
  then no orphaned-server sweep runs and the embedded backend is never imported
