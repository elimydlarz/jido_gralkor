...
when a turn is appended for a session that holds none
  ...
  and the group it binds is the sanitized form of the group supplied
...
if the flush callback raises or fails for any other reason
  ...
  and once the schedule is exhausted the turns are dropped and an exhausted line is logged at error
