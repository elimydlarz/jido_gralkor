...
when the memory add tool runs with content and a source description
  then it returns an acknowledgement immediately, without waiting on the write
  and the write is carried out in the background under the operator's sanitised group id, carrying the content and source description as given
